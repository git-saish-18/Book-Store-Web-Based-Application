...
def signup():
    if request.method == "POST":
        ...
        if email in email_data:
            return "<script LANGUAGE='JavaScript'>window.alert('Email Already Exists!');window.location.href='/login';</script>"
            email_file.close()
        else:
...
def add():
    ...
    return "<script LANGUAGE='JavaScript'>window.alert('Cannot access this endpoint');window.location.href='/';</script>"
...
def handle(error):
    return "<script LANGUAGE='JavaScript'>window.alert('Server Side ERROR! ⚠️ ');window.location.href='/';</script>"
